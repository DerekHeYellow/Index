...
for x in urend:
	...
	#weighted by market cap. This time we used a thresholding technique as described in the Methodology.
	thresholds = [0, 1000000000, 2000000000]
	thrsh = 2
	while thresholds[thrsh] < 300000000000:
		thresholds.append(thresholds[thrsh]+thresholds[thrsh - 1])
		thrsh = thrsh + 1
	end = len(thresholds)
	multiplier = 1
	count2 = 0
	...
	while thrsh2 <= end - 2:
		for y in MC100:
			if y >= thresholds[thrsh2] and y < thresholds[thrsh2 + 1]:
				tMC = tMC + y
		while count2 < 49 and MC100[count2] < thresholds[thrsh2 + 1]:
...
plt.title('1 Year Weekly CryptoCurrency Index Using Thresholding')
